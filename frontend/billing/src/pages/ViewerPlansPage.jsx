...
const statusConfig = {
    paid: { label: 'Paid', dotClass: 'bg-emerald-500', badgeClass: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20', animate: false },
    pending: { label: 'Pending', dotClass: 'bg-amber-500', badgeClass: 'bg-amber-500/10 text-amber-400 border-amber-500/20', animate: true },
};
...
export default function ViewerPlansPage() {
    ...
    useEffect(() => {
        const fetchInvoices = async () => {
            try {
                const res = await fetch(`${API_BASE}/api/viewer/invoices`, { headers: getAuthHeaders() });
    ...
    const filteredInvoices = useMemo(() => {
        ...
        if (searchQuery.trim()) {
            ...
            results = results.filter((inv) => inv.id.toLowerCase().includes(q) || inv.amount.toLowerCase().includes(q) || inv.amount.replace('₹', '').toLowerCase().includes(q));
        }
        if (statusFilter !== 'all') results = results.filter((inv) => inv.status === statusFilter);
        if (dateFilter !== 'all') {
        ...
        return results;
    }, [allInvoices, searchQuery, statusFilter, dateFilter]);

    useMemo(() => { setCurrentPage(1); }, [searchQuery, statusFilter, dateFilter]);
    ...
    if (loading) {
        return (
            <div className="flex-1 w-full max-w-[1200px] mx-auto px-4 md:px-8 py-8 md:py-12 flex items-center justify-center min-h-[60vh]">
            ...
            <div className="bg-surface-dark rounded-[24px] shadow-lg border border-slate-800 overflow-hidden flex flex-col min-h-[600px]">
                <div className="p-6 border-b border-slate-800 flex flex-col sm:flex-row gap-4 justify-between items-center bg-surface-dark">
                    <div className="relative w-full sm:w-96 group">
                        ...
                        <input className="w-full pl-10 pr-4 py-2.5 bg-background-dark border border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm placeholder:text-slate-500 text-white" placeholder="Search by Invoice ID or Amount..." type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                    </div>
                    <div className="flex items-center gap-3 w-full sm:w-auto">
                        <div className="relative" ref={dateRef}>
                            <button onClick={() => { setShowDateDropdown(!showDateDropdown); setShowStatusDropdown(false); }} className={`flex items-center gap-2 px-4 py-2.5 border rounded-lg text-sm font-medium transition-all shadow-sm ${dateFilter !== 'all' ? 'bg-primary/10 border-primary/30 text-primary' : 'bg-background-dark border-slate-700 text-slate-300 hover:bg-slate-800 hover:border-slate-600'}`}>
                            ...
                            {showDateDropdown && (
                                <div className="absolute right-0 top-full mt-2 w-48 bg-surface-dark rounded-xl shadow-xl border border-slate-700 py-2 z-50">
                                    {dateRanges.map((range) => (
                                        <button key={range.value} onClick={() => { setDateFilter(range.value); setShowDateDropdown(false); }} className={`w-full text-left px-4 py-2.5 text-sm font-medium transition-colors ${dateFilter === range.value ? 'bg-primary/10 text-primary' : 'text-slate-300 hover:bg-slate-800'}`}>{range.label}</button>
                                    ))}
                                </div>
                            )}
                        </div>
                        <div className="relative" ref={statusRef}>
                            <button onClick={() => { setShowStatusDropdown(!showStatusDropdown); setShowDateDropdown(false); }} className={`flex items-center gap-2 px-4 py-2.5 border rounded-lg text-sm font-medium transition-all shadow-sm ${statusFilter !== 'all' ? 'bg-primary/10 border-primary/30 text-primary' : 'bg-background-dark border-slate-700 text-slate-300 hover:bg-slate-800 hover:border-slate-600'}`}>
                            ...
                            {showStatusDropdown && (
                                <div className="absolute right-0 top-full mt-2 w-40 bg-surface-dark rounded-xl shadow-xl border border-slate-700 py-2 z-50">
                                    {[{ label: 'All', value: 'all' }, { label: 'Paid', value: 'paid' }, { label: 'Pending', value: 'pending' }].map((opt) => (
                                        <button key={opt.value} onClick={() => { setStatusFilter(opt.value); setShowStatusDropdown(false); }} className={`w-full text-left px-4 py-2.5 text-sm font-medium transition-colors ${statusFilter === opt.value ? 'bg-primary/10 text-primary' : 'text-slate-300 hover:bg-slate-800'}`}>{opt.label}</button>
                                    ))}
                                </div>
                            )}
                        </div>
                        ...
                        <tbody className="divide-y divide-slate-800 text-sm">
                            {paginatedInvoices.length === 0 ? (
                                ...
                                paginatedInvoices.map((invoice) => {
                                    const config = statusConfig[invoice.status] || statusConfig.paid;
                                    return (
                                        <tr key={invoice.id} className="group hover:bg-slate-700/30 transition-colors">
                                            ...
                                            <td className="py-4 px-6 text-right">
                                                <button onClick={() => downloadInvoice(invoice)} className="inline-flex items-center justify-center p-2 rounded-lg text-slate-500 hover:text-primary hover:bg-primary/10 transition-colors" title="Download Invoice">
                ...
                <div className="p-6 border-t border-slate-800 flex items-center justify-between bg-surface-dark">
                    <span className="text-sm text-slate-500">
                        {filteredInvoices.length === 0 ? 'No results' : (<>Showing <span className="font-medium text-white">{showingStart}-{showingEnd}</span> of <span className="font-medium text-white">{filteredInvoices.length}</span> invoices</>)}
                    </span>
                    <div className="flex items-center gap-2">
                        <button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1} className="px-3 py-1.5 rounded-lg border border-slate-700 text-sm font-medium text-slate-400 hover:bg-slate-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">Previous</button>
                        <span className="text-sm text-slate-500 px-2">Page <span className="text-white font-medium">{currentPage}</span> of <span className="text-white font-medium">{totalPages}</span></span>
                        <button onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="px-3 py-1.5 rounded-lg border border-slate-700 text-sm font-medium text-slate-300 hover:bg-slate-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">Next</button>
                    </div>
                </div>
            </div>
